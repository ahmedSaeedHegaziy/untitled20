...
int count = 0;
List<Mail> allCompleted = [];

class MyGroupListCompleted extends StatefulWidget {

  final List<dynamic> mails;
  final List<dynamic> categories;
  final bool showCompleted;
  const MyGroupListCompleted(
      {Key? key,
        required this.mails,
  ...
  @override
  State<MyGroupListCompleted> createState() => _MyGroupListState();
}

class _MyGroupListState extends State<MyGroupListCompleted> {
  String userData = " ";
  ...
  Future<List<Mail>> getUserData() async {
    ...
    if (response.error == null) {
      ...
      allCompleted = inbox
          ...
          .toList();
      count = allCompleted.length; // Update the count value
      print("all : ${allCompleted.length}");

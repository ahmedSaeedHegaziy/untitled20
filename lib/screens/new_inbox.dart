...
import 'package:untitled20/models_localhost/category.dart';
import 'package:untitled20/models_localhost/sender.dart';
...
import 'package:untitled20/screens/category_selector.dart';
import 'package:untitled20/screens/status_selector.dart';
import 'package:untitled20/screens/status_admin.dart';
...
class _NewInboxState extends State<NewInbox> {
  ...
  void pickImage() async {
    ...
    setState(() {
      images!.add(image); // Add the picked image to the images list.
    });
  }

  void sendMail() async {

    var reciverAndPhone = <String>[]; // Initialize as an empty list of strings
    addusers.forEach((key, value) {
      reciverAndPhone.add('${value.name}:${value.mobile}#');
    });
    String subAndReciver = _titleCnt.text + "|" + reciverAndPhone.join();
    ...
    Map<String, dynamic> mailData = {
      ...
      // Add other required data for the email.
    };

    // Call the sendMail function and pass the mailData and images list.
    ApiResponse response = await createMail(mailData, images);
    ...
    if (response.error != null) {
      ...
      // You can access the response data using response.data if needed.
    ...
    await showModalBottomSheet(
      ...
      builder: (context) => AllUsers(),
        ...
        setState(() {
          if (value['founded']) {
            user = value['user'];
            _senderCnt.text = addusers[user!.id] != null ? addusers[user!.id].name : " ";
            _mobileCnt.text = user?.mobile ?? '';
            category = value['user'].category;
          } else {
            sender = null;
            category = null;
            _senderCnt.text = value['user'];
            _mobileCnt.text = '';
          }
        }));
  }
  ...
  void showSnackBar(BuildContext context) {
    final snackBar = SnackBar(
      content: Text('Addition succeeded').tr(),
